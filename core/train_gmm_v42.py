# train_gmm_v42.py - Hierarchical & Hybrid Feature Deepfake Detection (Fixed v4)
# Author: Master Happy (Fixes integrated by ChatGPT)
# Date: 2025-10-23
#
# Fixes:
#  - Corrected AUC computation (removed double-negation)
#  - Uses LR probabilities for ROC-AUC
#  - Improved GMM convergence stability
#  - Added sanity checks for score inversion
#  - General cleanup & better logging consistency
...
# ---------------- CONFIG ----------------
NUM_TRIALS = 1
MODEL_PREFIX = "models/hier_gmm_model_v42"
RESULTS_PATH = "results/hier_gmm_results_v42.json"
SUBSAMPLE_LIMIT = 50000
CACHE_DIR = "cache_v42"
LOG_PATH = "results/train_gmm_v42_log.txt"
...
# ---------------- METRICS ----------------
def compute_metrics(labels, preds, proba):
...
def fuse_features_by_paths(residual_feats, freq_feats, lbp_feats, cnn_feats,
    ...
    res_array = zscore_block(np.array(residual_feats))
    freq_array = zscore_block(np.array(freq_feats))
    lbp_array = zscore_block(np.array(lbp_feats))
    cnn_array = zscore_block(np.array(cnn_feats))
    ...
    return np.array(fused_list), np.array(y_aligned_list)
...
if __name__=="__main__":
    ...
    train_proba = lr_model.predict_proba(train_scores.reshape(-1,1))[:,1]
    val_proba = lr_model.predict_proba(val_scores.reshape(-1,1))[:,1]
    test_proba = lr_model.predict_proba(test_scores.reshape(-1,1))[:,1]

    # Sanity check for potential inversion
    for name, y_true, proba in [("train", y_train_aligned, train_proba),
                                ("val", y_val_aligned, val_proba),
                                ("test", y_test_aligned, test_proba)]:
        auc_tmp = roc_auc_score(y_true, proba)
        if auc_tmp < 0.5:
            log(f"⚠️ {name.upper()} AUC below 0.5 ({auc_tmp:.4f}). Possible label inversion.")
        else:
            log(f"{name.upper()} AUC sanity check OK: {auc_tmp:.4f}")

    # ---------------- Compute Metrics ----------------
    train_metrics = compute_metrics(y_train_aligned, train_pred, train_proba)
    val_metrics = compute_metrics(y_val_aligned, val_pred, val_proba)
    test_metrics = compute_metrics(y_test_aligned, test_pred, test_proba)
